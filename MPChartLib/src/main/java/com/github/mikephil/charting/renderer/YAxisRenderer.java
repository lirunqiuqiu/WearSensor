...
public class YAxisRenderer extends AxisRenderer {
    ...
    public void renderScaleLines(Canvas c) {
        ...
        for (int i = positions.length; i > 2; i -= 2) {
            // 偏移量
            float offset = (positions[i - 1] - positions[i - 3]) / 5;
    ...
    protected void drawScale(Canvas canvas, float startY, float offset) {
        ...
        // Y轴在右边的位置
        float rightX = mViewPortHandler.contentRight();
        canvas.save();
        // Y轴位置在左边时
        if (mYAxis.getAxisDependency() == AxisDependency.LEFT) {
            for (int i = 0; i <= 5; i++) {
                ...
                if (i % 5 == 0) {
                    // 画长刻度线
                    canvas.drawLine(leftX, startY, leftX + 20, startY, mAxisLinePaint);
                } else {
                    // 画短刻度线
                    canvas.drawLine(leftX, startY, leftX + 10, startY, mAxisLinePaint);
                }
                canvas.restore();
            }
        }
        ...
        if (mYAxis.getAxisDependency() == AxisDependency.RIGHT) {
            for (int i = 0; i <= 5; i++) {
                ...
                canvas.translate(0, offset * i);
                if (i % 5 == 0) {
                    canvas.drawLine(rightX, startY, rightX - 20, startY, mAxisLinePaint);
                } else {
                    canvas.drawLine(rightX, startY, rightX - 10, startY, mAxisLinePaint);
                }
                canvas.restore();
            }
        }
